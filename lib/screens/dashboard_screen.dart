...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void _handleLevelDown(int oldLevel, int newLevel) {
    if (!mounted) return;
    setState(() {
      _showLevelDownBanner = true;
      _levelDownFrom = oldLevel;
      _levelDownTo = newLevel;
    });
    HapticFeedback.heavyImpact();
    SoundService.playError();
    _levelDownController.forward(from: 0);
    NotificationService.sendLevelDownNotification(
  ...
  void _updateLeaderboardLocalCacheOnly() {
    ...
    if (user == null) {
      _leaderboardRankCache = -1;
      _leaderboardScoreCache = 0.0;
  ...
  Future<void> _refreshLeaderboardCacheIfNeeded(
  ...
  int _completedToday() =>
      _habits.where((e) => e.isCompletedToday()).length;
      ...
      card = AnimatedBuilder(
        ...
        builder: (ctx, child) {
          return Transform.scale(
            scale: _habitCelebrationScale.value,
            child: Stack(
  ...
  Widget _buildTreeHealthPill(bool isDark) {
    ...
    final health = (((level / 20.0).clamp(0.0, 1.0) * 0.35) +
        ...
        .clamp(0.0, 1.0);
    String e = '🌱';
    Color c = const Color(0xFF86EFAC);
    if (health >= 0.75) {
      e = '🌴';
  ...
  Widget _buildQuoteCard(bool isDark) {
    return AnimatedBuilder(
        animation: _quoteFloatController,
        builder: (ctx, _) => Transform.translate(
            offset: Offset(0, _quoteFloat.value),
            child: _buildGlassContainer(
                isDark: isDark,
                padding: const EdgeInsets.symmetric(
  ...
  Widget _buildHabitsHeader(bool isDark) {
    return Row(children: [
...
class _PremiumProgressPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (progress > 0.02) {
      ...
      final dy = center.dy + radius * sin(ea);
      canvas.drawCircle(
          Offset(dx, dy),
          strokeWidth * 1.0,
          Paint()
