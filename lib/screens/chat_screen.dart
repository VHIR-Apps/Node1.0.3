...
class _ChatScreenState extends State<ChatScreen>
  ...
  void initState() {
    ...
    _isBlocked =
        LeaderboardModerationService.isBlocked(widget.peerUid);

    // Chat screen খোলা থাকলে এই peer-এর notification mute করো
    ChatNotificationService.instance
        .setActiveChatPeer(widget.peerUid);

    // এই peer-এর pending notification cancel করো
    ChatNotificationService.instance
        .cancelChatNotification(widget.peerUid);
  }
  ...
  void dispose() {
    ...
    // Chat screen বন্ধ হলে mute তুলে নাও
  ...
  Future<void> _playChatSound(String fileName) async {
    ...
    try {
      ...
      await _chatAudioPlayer
          .play(AssetSource('sounds/$fileName'));
    } catch (e) {
  ...
  Future<void> _sendMessage(
      {bool isChallenge = false}) async {
    if (_isBlocked) {
      _showSnack('You have blocked this user.',
          isError: true);
      return;
    }
    ...
    final msgText = isChallenge && text.isEmpty
            ...
            children: [
              ...
              IconButton(
                onPressed: () async {
                  ...
                  if (confirm == true) {
                    await ChatService.instance
                        .clearChatHistory(widget.peerUid);
                    if (mounted) {
  ...
  Widget _buildMessageBubble(
      ChatMessage msg, bool isMe, bool isDark) {
    final align =
      ...
      body: Stack(
        children: [
                      ...
                      return Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                                'Start a conversation or send a challenge.',
                                style: TextStyle(
                                    color: isDark
                                        ? Colors.white54
                                        : Colors.black54,
          ...
          Positioned(
              ...
              left: 0,
              right: 0,
              child: _buildGlassAppBar(isDark)),
        ],
      ),
    );
  }
