...
import 'package:path_provider/path_provider.dart' as path_provider; // 🚀 FIX: Added for safe background execution
...
class _PsychEngine {
  ...
  static String buildTitle({
    ...
    if (isMissed) return "${habit.emoji} Comeback Time: ${habit.name}";
    if (hour < 11) return "${habit.emoji} Morning Target: ${habit.name}";
    if (hour >= 22) return "🚨 CRITICAL: ${habit.name} is pending!";
    if (streak >= 30) return "${habit.emoji} 🔥 $streak-Day Warrior: ${habit.name}";
    if (streak >= 7) return "${habit.emoji} 🔥 $streak-Day Streak: ${habit.name}";
    if (streak > 0) return "${habit.emoji} Protect the Streak: ${habit.name}";
...
class NotificationService {
  ...
  static String _getEmojiForType(String type) {
    switch (type.toLowerCase()) {
      case 'achievement': case 'milestone': return '🏆';
      case 'streak': case 'fire': return '🔥';
      case 'reminder': return '🔔';
      case 'smart': return '💡';
      case 'psychology': return '🧠';
      case 'summary': return '📊';
      case 'alarm': return '⏰';
      case 'warning': case 'missed': return '⚠️';
      case 'level_down': return '📉';
      case 'system': return '⚙️';
      case 'pomodoro': case 'study': case 'timer': return '📚';
      default: return '💬';
    }
  }
  ...
  static Future<void> init() async {
    ...
    if (androidPlugin != null) {
      await androidPlugin.createNotificationChannel(const AndroidNotificationChannel(
        channelHabitReminders, 'Habit Reminders', importance: Importance.max, playSound: true, enableVibration: true,
      ));
      await androidPlugin.createNotificationChannel(const AndroidNotificationChannel(
        channelDailySummary, 'Daily Summary', importance: Importance.high, playSound: false, enableVibration: false,
      ));
      await androidPlugin.createNotificationChannel(const AndroidNotificationChannel(
        channelInstant, 'General Notifications', importance: Importance.max, playSound: true, enableVibration: true,
      ));
      await androidPlugin.createNotificationChannel(const AndroidNotificationChannel(
        channelSmartReminders, 'Smart Reminders', importance: Importance.max, playSound: true, enableVibration: true,
      ));
      await androidPlugin.createNotificationChannel(const AndroidNotificationChannel(
        channelHabitAlarm, 'Habit Alarms', importance: Importance.max, playSound: true, enableVibration: true, enableLights: true,
      ));
      await androidPlugin.createNotificationChannel(const AndroidNotificationChannel(
        channelPsychology, 'Motivation Nudges', importance: Importance.max, playSound: true, enableVibration: true,
      ));
    }
  ...
  // ─────────────────────────────────────────────
  // 🎯 SMART NOTIFICATION TAP HANDLER (FOREGROUND)
  // ─────────────────────────────────────────────
  ...
  static void _handleBackgroundNotificationTap(NotificationResponse response) async {
    ...
    // "✅ I Did It" বা "Dismiss" চাপলে অ্যাপ ওপেন না করেই ডেটাবেসে কাজ করবে
    if (actionId == 'mark_done' || actionId == 'dismiss') {
      try {
        WidgetsFlutterBinding.ensureInitialized();

        // 🚀 FIX: Use path_provider to safely initialize Hive in background isolate
        final dir = await path_provider.getApplicationDocumentsDirectory();
        Hive.init(dir.path);

        if (!Hive.isAdapterRegistered(0)) Hive.registerAdapter(HabitAdapter());

        final cleanPayload = payload.startsWith('alarm:') ? payload.replaceFirst('alarm:', '') : payload;
        ...
        if (targetHabit != null && !targetHabit.isCompletedToday()) {
          ...
          debugPrint('✅ Background task complete: Habit Marked as Done!');
        }
        await box.close();
      } catch (e) {
  ...
  static Future<void> scheduleHabitReminder(Habit habit, {bool isSilent = false}) async {
    ...
    if (habit.reminderEnabled) await _scheduleDailyNotification(habit: habit, timeStr: habit.time);
  ...
  static Future<void> _scheduleDailyNotification({required Habit habit, required String? timeStr}) async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelHabitReminders, 'Habit Reminders',
      importance: Importance.max, priority: Priority.max,
      icon: _notificationIcon, color: Color(habit.colorValue),
      enableVibration: true, playSound: true, autoCancel: true,
      ...
      actions: const [
        // 🚀 showsUserInterface: false মানে ফোন লক থাকলেও কাজ হবে
    ...
    try {
      await _plugin.zonedSchedule(
        ...
        payload: habit.id, matchDateTimeComponents: DateTimeComponents.time,
      );
    } catch (_) {}
  }
  ...
  static Future<void> _scheduleFullScreenAlarm({required Habit habit}) async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelHabitAlarm, 'Habit Alarms',
      importance: Importance.max, priority: Priority.max,
      icon: _notificationIcon, color: Color(habit.colorValue),
      enableVibration: true, playSound: true, autoCancel: false, ongoing: true,
    ...
    try {
      await _plugin.zonedSchedule(
        ...
        uiLocalNotificationDateInterpretation: UILocalNotificationDateInterpretation.absoluteTime,
        payload: 'alarm:${habit.id}', matchDateTimeComponents: DateTimeComponents.time,
      );
    } catch (_) {}
  }
  ...
  static Future<void> sendPsychologyNudge(Habit habit) async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelPsychology, 'Motivation Nudges',
      importance: Importance.max, priority: Priority.max,
      icon: _notificationIcon, color: const Color(0xFFEF4444), enableVibration: true, playSound: true,
  ...
  static Future<void> sendPsychologyNudgesForIncompleteHabits() async {
    if (!DatabaseService.areNotificationsEnabled()) return;
    for (final habit in DatabaseService.getAllHabits()) {
      if (!habit.isCompletedToday() && habit.isActiveToday()) {
  ...
  static Future<void> scheduleEveningPsychologyNudges() async {
    ...
    for (final habit in DatabaseService.getAllHabits()) {
      ...
      if (evening.isBefore(now) || (habit.isCompletedToday() && evening.day == now.day)) {
      ...
      final androidDetails = AndroidNotificationDetails(
        channelPsychology, 'Motivation Nudges',
        importance: Importance.max, priority: Priority.max,
        icon: _notificationIcon, color: const Color(0xFF6C63FF),
        styleInformation: BigTextStyleInformation(message, htmlFormatBigText: true, contentTitle: '<b>$title</b>', htmlFormatContentTitle: true),
        actions: const [
      ...
      final exactAllowed = await canScheduleExactAlarms();
      try {
        await _plugin.zonedSchedule((habit.id.hashCode.abs() + 95000) % 100000, title, message, evening, NotificationDetails(android: androidDetails),
          androidScheduleMode: exactAllowed ? AndroidScheduleMode.exactAllowWhileIdle : AndroidScheduleMode.inexactAllowWhileIdle,
          uiLocalNotificationDateInterpretation: UILocalNotificationDateInterpretation.absoluteTime, payload: habit.id,
        );
      } catch (_) {}
    }
  }
  ...
  static Future<void> sendXpLossWarning({required Habit habit, required int xpAtRisk}) async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelPsychology, 'Motivation Nudges',
      importance: Importance.max, priority: Priority.max,
      icon: _notificationIcon, color: const Color(0xFFEF4444),
      styleInformation: BigTextStyleInformation(body, htmlFormatBigText: true, contentTitle: '<b>$title</b>', htmlFormatContentTitle: true),
      actions: const [
  ...
  static Future<void> scheduleSnoozeAlarm({required Habit habit, Duration delay = const Duration(minutes: 5)}) async {
    ...
    try {
      await _plugin.zonedSchedule(
        _snoozeAlarmId(habit.id), '💤 Snoozed: ${habit.emoji} ${habit.name}', 'Time to act! Don\'t skip this habit.', scheduledTime,
        NotificationDetails(android: androidDetails),
        androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle,
        uiLocalNotificationDateInterpretation: UILocalNotificationDateInterpretation.absoluteTime, payload: 'alarm:${habit.id}',
      );
    } catch (_) {}
  }
  ...
  static Future<void> sendSmartReminder(Habit habit) async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelSmartReminders, 'Smart Reminders',
      importance: Importance.max, priority: Priority.max,
      icon: _notificationIcon, color: Color(habit.colorValue), enableVibration: true, playSound: true,
      styleInformation: BigTextStyleInformation(message, htmlFormatBigText: true, contentTitle: '<b>$title</b>', htmlFormatContentTitle: true),
      actions: const [
        AndroidNotificationAction('mark_done', '✅ Done', showsUserInterface: false, cancelNotification: true),
  ...
  static Future<void> showInstantNotification({required String title, required String body, String type = 'local', String? payload, bool saveToDb = true}) async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelInstant, 'General Notifications',
      importance: Importance.max, priority: Priority.max,
      icon: _notificationIcon, color: const Color(0xFF6C63FF),
  ...
  static Future<void> showPomodoroNotification({required String title, required String body}) async {
    ...
    await _plugin.show(DateTime.now().millisecondsSinceEpoch % 100000, '📚 $title', body, NotificationDetails(android: androidDetails), payload: 'pomodoro');
    await _saveNotificationToDb(title: '📚 $title', body: body, type: 'pomodoro', payload: 'pomodoro');
  }
  ...
  static Future<void> scheduleDailySummary() async {
    ...
    final androidDetails = AndroidNotificationDetails(
      channelDailySummary, 'Daily Summary',
      importance: Importance.high, priority: Priority.high,
      icon: _notificationIcon, color: const Color(0xFF6C63FF), playSound: true, enableVibration: true,
      styleInformation: BigTextStyleInformation(body, htmlFormatBigText: true, contentTitle: '<b>$title</b>', htmlFormatContentTitle: true),
    );
    ...
    try {
      await _plugin.zonedSchedule(
        ...
        androidScheduleMode: exactAllowed ? AndroidScheduleMode.exactAllowWhileIdle : AndroidScheduleMode.inexactAllowWhileIdle,
        uiLocalNotificationDateInterpretation: UILocalNotificationDateInterpretation.absoluteTime, payload: 'daily_summary',
      );
    } catch (_) {}
  }
}